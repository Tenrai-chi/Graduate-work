...
max_operations = 4  # Количество максимальных операций

all_of_task = []  # История созданных логических выражений
...
def generation(max_operation: int) -> str:
    """ Генерирует логическое выражение и возвращает его в виде строки """

    operation = exam_a = exam_b = exam_c = 0  # Обновление всех счетчиков
    ...
    all_of_task.append(task_original11)  # Добавление в "историю"
...
def matrix_calculation(size: int) -> tuple:
    """ Рассчет и заполнение матрицы истинности операндов
        Возвращает матрицу, высоту и ширину матрицы """
...
def solution() -> list:
    """ Решает матрицу и возвращает ее """
...
def show_matrix() -> list:
    """ Выводит матрицу на экран, а затем
        возвращает ее для последующей обработки """
...
def examination() -> list:
    """ Создает матрицу, с которой будет работать пользователь
        ТРЕБУЕТ ИЗМЕНЕНИЙ ПРИ СОЗДАНИИ ИНТЕРФЕЙСА """
...
def check_results():
    """ Сравнение 2 матриц по последнему эллементу каждой строки """
